...
impl GitOpt {
    ...
    pub fn commit(
        ...
        external_versions: &Map<Utf8PathBuf, Map<String, Version>>,
    ...
    fn commit_msg(
        ...
        external_versions: &Map<String, Version>,
    ) -> String {
        let mut sections = vec![];

        if !new_versions.is_empty() {
            sections.push(self.commit_msg_section("Local Packages", new_versions));
        }

        if !external_versions.is_empty() {
            sections.push(self.commit_msg_section("External Packages", external_versions));
        }
    ...
    fn commit_msg_section(&self, title: &str, versions: &Map<String, Version>) -> String {
        format!(
            "{}:\n{}",
            title,
            versions
    ...
    fn commit_version_label(
        ...
        if root != workspace_root && repo_versions.len() == 1 {
            repo_versions
                .iter()
                .next()
                .map(|(pkg_name, data)| {
    ...
    fn individual_tag(
        ...
        let prefix = if root != workspace_root {
            &self.tag_prefix
        } else {
...
#[cfg(test)]
mod tests {
    use super::{GitOpt, Map, RepoVersion};
    ...
    fn individual_tag_uses_global_prefix_for_independent_package_in_separate_repo() {
        ...
        let tag = git.individual_tag(&workspace_root, &root, "crate", &version);
    ...
    fn individual_tag_uses_package_prefix_for_independent_package_in_workspace_repo() {
        ...
        let tag = git.individual_tag(&workspace_root, &workspace_root, "crate", &version);

        assert_eq!(tag, "crate@1.2.3");
    }

    #[test]
    fn individual_tag_uses_global_prefix_for_non_independent_package_in_separate_repo() {
        let git = git_opt();
        let workspace_root = Utf8PathBuf::from("/workspace");
        let root = Utf8PathBuf::from("/workspace/submodule");
        let version = Version::parse("1.2.3").expect("valid version");
    ...
    #[test]
    fn commit_version_label_uses_package_prefix_for_single_package_in_workspace_repo() {
        let git = git_opt();
        let workspace_root = Utf8PathBuf::from("/workspace");
        let mut repo_versions = Map::new();

        repo_versions.insert(
            "crate".to_string(),
            RepoVersion {
                version: Version::parse("1.2.3").expect("valid version"),
                root: false,
    ...
    #[test]
    fn commit_version_label_uses_global_prefix_for_single_package_in_separate_repo() {
        let git = git_opt();
        let workspace_root = Utf8PathBuf::from("/workspace");
        let root = Utf8PathBuf::from("/workspace/submodule");
        let mut repo_versions = Map::new();
        ...
        let label = git.commit_version_label(&workspace_root, &root, &None, &repo_versions);
    ...
    #[test]
    fn commit_version_label_uses_custom_global_prefix_for_single_package_in_separate_repo() {
        let mut git = git_opt();
        let workspace_root = Utf8PathBuf::from("/workspace");
        let root = Utf8PathBuf::from("/workspace/submodule");
        let mut repo_versions = Map::new();
        ...
        assert_eq!(label, "release-1.2.3");
    }

    #[test]
    fn commit_msg_separates_local_and_external_packages() {
        let git = git_opt();
        let mut local_versions = Map::new();
        let mut external_versions = Map::new();
        ...
        external_versions.insert(
            "external".to_string(),
            Version::parse("2.3.4").expect("valid version"),
        );

        let msg = git.commit_msg("Release 1.2.3", &local_versions, &external_versions);
    ...
    #[test]
    fn commit_msg_omits_empty_sections() {
        let git = git_opt();
        let external_versions = Map::new();
        let mut local_versions = Map::new();
